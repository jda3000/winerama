...
def user_review_list(request, username=None):
    ...
    context = {'latest_review_list':latest_review_list, 'username':username, 'name':name }
...
def user_recommendation_list(request):
    ...
    try:
        # get the request user cluster name (just the first one right now)
        user_cluster_name = User.objects.get(username=request.user.username).cluster_set.first().name
    except:
        update_clusters()
        user_cluster_name = User.objects.get(username=request.user.username).cluster_set.first().name

    # get usernames for other members of hte cluster
    ...
    # get reviews by those users, exluding wines reviews by hte user
    other_users_review = Review.objects.filter(user_name__in=other_members_usernames).exclude(wine__id__in=user_reviews_wine_ids)
    other_users_reviews_wine_ids = set(map(lambda x: x.wine.id, other_users_review))
